...
def create_default_groups():
    Group.objects.get_or_create(name="Admin", id=3)
...
from .serializers import DeveloperRequestSerializer
from rest_framework.decorators import api_view, permission_classes
...
import string
@api_view(['POST'])
@permission_classes([AllowAny])  # public can request
def developer_request_create(request):
    serializer = DeveloperRequestSerializer(data=request.data)
    if serializer.is_valid():
        serializer.save()
        return Response(serializer.data, status=status.HTTP_201_CREATED)
    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
@api_view(['GET'])
def developer_request_list(request):
    requests = DeveloperRequest.objects.all()
    serializer = DeveloperRequestSerializer(requests, many=True)
    return Response(serializer.data)

@api_view(['PATCH'])
def approve_developer_request(request, request_id):
    try:
        dev_request = DeveloperRequest.objects.get(id=request_id)
    except DeveloperRequest.DoesNotExist:
        return Response({'error': 'Request not found.'}, status=404)

    if dev_request.is_approved:
        return Response({'error': 'Already approved.'}, status=400)

    # generate random password
    password = ''.join(random.choices(string.ascii_letters + string.digits, k=8))
    user = User.objects.create_user(username=dev_request.email, email=dev_request.email, password=password)
    researcher_group, _ = Group.objects.get_or_create(name='Researcher')
    user.groups.add(researcher_group)
    user.save()

    dev_request.is_approved = True
    dev_request.save()

    send_mail(
        subject='Your Developer Account Details',
        message=f"Your account has been approved.\nUsername: {dev_request.email}\nPassword: {password}",
        from_email='einas.mansor@gmail.com',
        recipient_list=[dev_request.email],
        fail_silently=True,  # <- prevents crashing
    )

    return Response({'message': 'Approved and user created.'})


@api_view(['PATCH'])
def reject_developer_request(request, request_id):
    try:
        dev_request = DeveloperRequest.objects.get(id=request_id)
    except DeveloperRequest.DoesNotExist:
        return Response({'error': 'Request not found.'}, status=404)
    ...
    if dev_request.is_approved:
        return Response({'error': 'Already approved, cannot reject.'}, status=400)

    dev_request.delete()
    return Response({'message': 'Request rejected and deleted.'})
@api_view(['GET', 'POST'])
def admin_manage_requests(request):
    """
    GET → List pending requests.
    POST → Approve request, create user account.
    """
    if request.method == 'GET':
        requests = DeveloperRequest.objects.filter(is_approved=False)
        serializer = DeveloperRequestSerializer(requests, many=True)
        return Response(serializer.data)
    ...
    if request.method == 'POST':
        ...
        if dev_request.is_approved:
            return Response({'error': 'Request already approved.'}, status=400)
        ...
        # Normally: Send email with credentials → simplified here
        print(f"Developer account created. Username: {dev_request.email}, Password: {password}")

        # Inside admin_manage_requests view
        send_mail(
            subject='Your Developer Account Details',
            message=f"Your account has been approved.\nUsername: {dev_request.email}\nPassword: {password}",
            from_email='einas.mansor@gmail.com',
        ...
        return Response({'message': 'Developer approved and account created.', 'username': dev_request.email, 'password': password})
import zipfile
from django.core.files.base import ContentFile
from rest_framework.decorators import api_view, permission_classes
from rest_framework.permissions import IsAuthenticated
from rest_framework.response import Response
from api.models import Manuscript, ManuscriptFile
import zipfile
import os
from django.core.files.base import ContentFile
from rest_framework.decorators import api_view, permission_classes
...
def upload_folder_view(request, manuscript_id):
    ...
    ext = os.path.splitext(archive.name)[1].lower()
    image_count = 0

    if ext == ".zip":
        with zipfile.ZipFile(archive) as zf:
            for name in zf.namelist():
                if name.endswith('/') or not os.path.basename(name):
                    continue
                if not name.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.tif', '.tiff', '.gif')):
                    continue
                content = zf.read(name)
                django_file = ContentFile(content, name=os.path.basename(name))
                ManuscriptFile.objects.create(manuscript=manuscript, file=django_file, file_type='image')
                image_count += 1
        ...
        with rarfile.RarFile(archive) as rf:
            for info in rf.infolist():
                if info.is_dir() or not os.path.basename(info.filename):
                    continue
                if not info.filename.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.tif', '.tiff', '.gif')):
                    continue
                try:
                    with rf.open(info) as file_data:
                        content = file_data.read()
                        django_file = ContentFile(content, name=os.path.basename(info.filename))
                        ManuscriptFile.objects.create(manuscript=manuscript, file=django_file, file_type='image')
                        image_count += 1
                except Exception as e:
        ...
        return Response({"error": "Only .zip or .rar files are supported"}, status=400)

    return Response({"message": f"{image_count} images uploaded successfully.", "files": image_count})
from django.http import FileResponse
from rest_framework.decorators import api_view, permission_classes
from rest_framework.permissions import IsAuthenticated
from django.shortcuts import get_object_or_404
from api.models import Manuscript
...
def download_manuscript_zip(request, manuscript_id):
    ...
    zip_buffer.seek(0)
    response = FileResponse(zip_buffer, as_attachment=True, filename=f"{manuscript.name}.zip")
    return response
